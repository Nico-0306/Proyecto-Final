...
class TaskManager:
  def__init__(self, root):
      ...
      self.task_tree= ttk.Treeview(task_frame, columns=("title", "due_date", "priority", "status", "tags"), show="headings")
      self.tasks_tree.heading("title", text="Título")
      self.tasks_tree.heading("due_date", text="Fecha Limite")
      ...
      self.tasks_tree.heading("title", width=200)
      self.tasks_tree.heading("due_date", width=100)
      self.tasks_tree.heading("priority", width=80)
      self.tasks_tree.heading("status", width=100)
      self.tasks_tree.heading("tags", width=150)
      ...
      elf.calendar = Calendar(calendar_frame, selectmode='day', date_pattern='yyyy-mm-dd')
      self.calendar.pack(fill=tk.BOTH, expand=True)
      
      #Visualizacion de tareas en el calendario 
...
def uplate_timeline_chart(self):
  self.ax.clear()

  if not self.filtered_task:
...
def apply_filters(self):
  ...
  for task in self.tasks:
    ...
    self.filtered_task.append(task)
